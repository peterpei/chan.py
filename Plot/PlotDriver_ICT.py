...
def handle_up(current_bi, current_clu):
    print("多")
    bi_1 = current_bi.pre
    result1 = CMarketStruct.INIT
    result2 = CMarketStruct.INIT
    text = ""
    if bi_1 is not None:
        if current_bi.get_end_val() > bi_1.get_begin_val():
            text += "HIGHER HIGH"
            print("HIGHER HIGH")
            result1 = CMarketStruct.HIGHER_HIGH
        else:
            if current_bi.is_sure:
                text += "\nHIGHER HIGH"
                print("HIGHER HIGH")
                result1 = CMarketStruct.HIGHER_LOW
            else:
                result1 = CMarketStruct.UNCONFIRMED
                print("【不确认】等")
                text += "\n【不确认】"

        bi_2 = bi_1.pre
        if result1 != CMarketStruct.UNCONFIRMED:  # 笔确认
            if bi_2 is not None:
                if current_bi.get_begin_val() > bi_2.get_begin_val():
                    print("LOWER HIGH")
                    result2 = CMarketStruct.LOWER_HIGH
                else:
                    print("lower low")
                    result2 = CMarketStruct.LOWER_LOW
            ...
            if bi_2 is not None:
                if current_bi.get_begin_val() > bi_2.get_begin_val():
                    ...
                    bi_3 = bi_2.pre
                    if bi_3 is not None:
                        if bi_1.get_end_val() > bi_3.get_begin_val():
                            # 没有跌破颈线位
                            print("没有跌破颈线位 【做多折扣区】寻找fvg做多")
                        else:
                            print("【跌破颈线位 注意观察】")
                else:
                    print("lower low")
                    ...
                    # distance = abs(bi_1.get_end_val() - bi_1.get_begin_val())
                    # pos = abs(current_clu.close - bi_1.get_end_val()) / distance
        ...
        if result1 == CMarketStruct.HIGHER_LOW and result2 == CMarketStruct.LOWER_LOW:
            print("LOWER_LOW,HIGHER_LOW")
            distance = abs(bi_1.get_end_val() - bi_1.get_begin_val())
            pos = abs(current_clu.close - bi_1.get_end_val()) / distance
            print(f"position:{pos:.3f}")
            if pos > 0.6:
        ...
        # 分离代码
        # if current_bi.get_end_val() > bi_1.get_begin_val():
        #     distance = abs(current_bi.get_end_val() - current_bi.get_begin_val())
        #     pos = abs(current_clu.close - current_bi.get_end_val()) / distance
        #     print(f"position:{pos:.3f}")
        #     if pos > 0.6:
        #         print("做多折扣区")
        #     elif pos < 0.4:
...
def handle_down(current_bi, current_clu):
    print("空")
    bi_1 = current_bi.pre
    result1 = CMarketStruct.INIT
    result2 = CMarketStruct.INIT
    ...
    if bi_1 is not None:
        if current_bi.get_end_val() < bi_1.get_begin_val():
            text += "LOWER LOW"
            print("LOWER LOW")
            result1 = CMarketStruct.LOWER_LOW
        else:
            if current_bi.is_sure:
                print("lower high")
                text += "\nlower high"
        ...
        if result1 != CMarketStruct.UNCONFIRMED:  # 笔确认
            if bi_2 is not None:
                if current_bi.get_begin_val() < bi_2.get_begin_val():
                    print("HIGHER LOW")
                    result2 = CMarketStruct.HIGHER_LOW
                else:
                    print("higher high")
                    result2 = CMarketStruct.HIGHER_HIGH
        else: # unconfirmed
            if bi_2 is not None:
                if current_bi.get_begin_val() > bi_2.get_begin_val():
                    print("LOWER HIGH")
...
class CPlotDriver_ICT:
    def __init__(self, chan: CChan, plot_config: Union[str, dict, list] = '', plot_para=None):
        ...
        # added by peter begin{
        ck_line = chan[chan.lv_list[0]]
        current_bi = ck_line.bi_list[-1]

        current_clu = current_bi.get_end_klu()
        # find last ku
        while current_clu.next is not None:
            current_clu = current_clu.next

        bi_1 = current_bi.pre

        if current_bi.is_down():
            if bi_1 is None:
                print("----只有一笔")
            else:  # b1 up
                handle_down(current_bi, current_clu)

        elif current_bi.is_up():
            if bi_1 is None:
                print("----只有一笔xu")
            else:  # b1 down
                handle_up(current_bi, current_clu)
    ...
    def draw_demark(
        ...
        for klu in meta.klu_iter():
            ...
            for demark_idx in klu.demark.get_countdown():
                box_bias = 0.5 * text_height if text_height is not None and demark_idx[
                    'idx'] == CDemarkEngine.MAX_COUNTDOWN else 0
                txt_instance = ax.text(
                    klu.idx,
